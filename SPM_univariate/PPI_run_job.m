...
% need to modify to use batch_sep_sessions_job.m for multi-session version

clear

cd /lustre/scratch/wbic-beta/ccn30/ENCRYPT/gridcellpilot/scripts/SPM_univariate
spm('defaults', 'FMRI');

Nscan = [238 238 238];

%% Create VOIs (note: could use SPM threshold to reduce voxels)
%for sess = 1:3
    jobfile = {'/lustre/scratch/wbic-beta/ccn30/ENCRYPT/gridcellpilot/scripts/SPM_univariate/batch_voi_job.m'};
    inputs = {};
   ...
   % inputs{3} = 1;
    inputs{3} = 'rightpmEC';
    inputs{4} = cellstr('/lustre/scratch/wbic-beta/ccn30/ENCRYPT/gridcellpilot/preprocessed_data/segmentation/28428/epimasks/pmEC_rightWarped_ITKaffine.nii'); % for single subject now
...
for sess = 1:3
    ...
    inputs{4} = 'leftpmEC';
    inputs{5} = cellstr('/lustre/scratch/wbic-beta/ccn30/ENCRYPT/gridcellpilot/preprocessed_data/segmentation/28428/epimasks/pmEC_leftWarped_ITKaffine.nii');
    spm_jobman('run', jobfile, inputs{:});
end
...
y1 = zscore(y1.Y); % source
y2 = load('VOI_ROI2_1.mat');
y2 = zscore(y2.Y); % target

load SPM
neoi = 3;
psyc = SPM.xX.X(:,1:neoi); % Care here - assumes single session at moment
